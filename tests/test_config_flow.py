...
async def test_user_step_success_auto_panel_id(hass):
    """panel_id is derived automatically from /discovery — user only enters host+PIN."""
    with aioresponses() as m:
        ...
        with patch("custom_components.elmax_local.config_flow."
            ...
            result = await hass.config_entries.flow.async_configure(
                result["flow_id"],
                {"panel_host": "1.2.3.4", "panel_pin": "000000"},
            )
        assert result["type"] == FlowResultType.CREATE_ENTRY
        assert result["data"][CONF_PANEL_ID] == "abc"
...
async def test_user_step_invalid_auth(hass):
    with aioresponses() as m:
        ...
        with patch("custom_components.elmax_local.config_flow."
            ...
            result = await hass.config_entries.flow.async_configure(
                result["flow_id"],
                {"panel_host": "1.2.3.4", "panel_pin": "wrong"},
            )
        assert result["type"] == FlowResultType.FORM
